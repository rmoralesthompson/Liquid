...
**Implementation note (#59).** Points settled while building the mode:

- **Two seams, both unexported `App` fields.** `App.now func() time.Time` (already present for idle-expiry tests) is joined by `App.rand io.Reader`, the opaque-token CSPRNG source. Both default to production values in `New` (`time.Now`, `crypto/rand.Reader`) and have **no exported mutator**, so a production consumer cannot reach them — determinism is injectable only from within package `liquid` (white-box tests) or the gated opt-in below. The free `randomToken()` became `(*App).newToken()`, reading `a.rand` via `io.ReadFull`; the CSRF secret is now drawn from `a.rand` *after* options apply, so pinning the source pins the secret too (production is byte-for-byte unchanged from the old `rand.Read`).
- **Opt-in gated like the dev surface.** `WithDeterminism()` (an `Option` pinning the clock to a frozen epoch and `a.rand` to a reproducible SHA-256-counter source) lives in `core/determinism_on.go` under `//go:build liquiddev`; `core/determinism_off.go` is a documented empty counterpart. A normal `go build` cannot name `WithDeterminism` — the "unreachable in a normal build" guarantee is compile-time, not runtime. Determinism means a *freshly constructed* deterministic `App` reproduces a render; one long-lived `App` still hands sibling components distinct hydro IDs.
- **Iteration order needed no new code.** The only framework map iteration that reaches output is `html/template`'s `{{range}}`, which already emits map keys in sorted order; `Event.Bind`'s payload-map iteration writes distinct struct fields and is order-independent. The byte-identical test ranges a map to pin this.
- **Two pins.** `core/determinism_wb_test.go` (untagged white-box, runs under `go test ./...`) sets all three inputs directly and asserts a render is byte-identical across two Apps; `core/determinism_off_test.go` (`//go:build !liquiddev`, twin of `dev_off_test.go`) asserts the default `App` uses `crypto/rand.Reader` and real time and that two production renders get distinct tokens. `core/determinism_on_test.go` (`//go:build liquiddev`) exercises the public `WithDeterminism()` end-to-end.
...
**Guard↔closed-domain coupling (#85, [ADR-0003](adr/0003-closed-domain-guard-coupling.md)).** A direct consequence of the anchoring above, decided as intended v0.1 behavior: a closed-domain enum field is enumerated — and thus enforced at the seam — **only when its action declares a guard**. Because the payload struct is named to the compiler solely through the guard's parameter, a closed-domain field on an *unguarded* action is invisible to `PayloadDomains` and unenforced; the author gets the LSX018 warning (its suggestion names this coupling), not seam enforcement. Writing the enum field is not, by itself, sufficient for an untyped handler — declare a guard (even `return true`) to opt in.

**Amended by #105 ([ADR-0004](adr/0004-typed-payload-handlers-and-validation.md)).** A handler may now take a **typed payload parameter** (`func (c *T) Submit(f Form)`), which names the payload type to the compiler directly — so a closed-domain field on a typed payload is enumerated and enforced **without a guard**, and an optional `Validate() liquid.Errors` runs at the seam. This is the recommended shape for any action carrying constrained values. The coupling persists only for the untyped `func(e liquid.Event)` case (its payload is still unnamed); `TestUnguardedClosedDomainIsNotEnforced` now pins that residual, and `TestTypedPayloadHandlerEnforcesClosedDomainWithoutGuard` pins the typed-payload fix.
