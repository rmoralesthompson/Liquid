...
| CSRF codec (D15) | `sessionID:expiry:HMAC-SHA256` over a delimited input, hex signature, constant-time compare; 32-byte per-process secret minted in `New` (panics rather than serving unsigned) | `core/csrf.go:22-57`, `core/app.go:164-169` | `core/csrf_test.go:13,27,38,49,62` (valid / foreign-session / expired / tampered / wrong-secret) |
| CSRF TTL tracks idle window (D15/D2) | Mint TTL is `Limits.SessionIdleTimeout`, not a constant | `core/app.go:458,556` | `TestCSRFTokenExpiryTracksTheIdleWindow` `core/hardening_test.go:92` |
| CSRF validated before dispatch (D15) | Refusal order in `/hydro-event`: missing cookie → 404, bad CSRF → 403, unknown token → 404, unknown action → 404; CSRF is checked **before** any registry lookup | `core/hydro.go:475-496` | `TestFireWithForgedCSRFTokenIsRefusedBeforeDispatch` `liquidtest/liquidtest_test.go:284` (+ `:304`, `:313`); `TestUnknownHydroTokenOrMissingSessionIs404` `core/hydro_test.go:227` — full 403-beats-404 sequence unpinned, → [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |
...
| Event body limits (D20.3) | Declared oversize → 413 before reading; body wrapped in `MaxBytesReader`; malformed JSON → 400 | `core/hydro.go:460-474` | `TestOversizedEventBodyIsRejectedBeforeDispatch` `core/hydro_test.go:484`; `TestMalformedEventBodyIs400` `:243` — mid-decode 413 path unpinned, → [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |
| Serialized dispatch (D20.1) | Per-**session** mutex serializes handler + re-render across all of a session's instances; one-way lock order (session `dispatch` → registry `mu`) | `core/hydro.go:174`, `:508-517`; pump takes the same mutex `core/sse.go:100-102` | `TestSameSessionEventsSerializeAcrossComponentInstances` `core/hydro_test.go:553`; `TestConcurrentEventsOnOneInstanceAllLand` `:589` (50 events under `-race`) |
| SSE bounds (D20) | Per-session stream cap (default 8, oldest evicted); slow consumers (16-frame buffer) disconnected, not served stale; pump goroutines die with their registry entry on every eviction/expiry path; sessionless streams refused | `core/hydro.go:92`, `core/sse.go:45,61-68,148-166,204-253` | `TestStreamsPerSessionAreBoundedByEvictingTheOldest` `liquidtest/sse_test.go:113`; `TestSlowStreamIsDisconnectedRatherThanBlocked` `core/sse_test.go:215`; pump reaping `core/sse_test.go:123,148,164`; `TestStreamWithoutALiveSessionIsRefused` `liquidtest/sse_test.go:54`; `TestReconnectYieldsCurrentStateWithNoMissedPatchReplay` `liquidtest/sse_test.go:140` |
...
| Guards before instantiation (D4/D19) | Guards run after route match, before the component is constructed; Deny → 403, Redirect → 302 | `core/guard.go:5-21`, `core/app.go:405-409`, `:480-494` | `TestDenyingGuardBlocksRenderWith403` `core/app_test.go:200`; `TestRedirectingGuardRespondsWithRedirect` `:228` — "never constructed" not directly pinned, → [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |
| Method gates | Page routes GET/HEAD only (405 otherwise); `/hydro-event` POST-only; `/hydro-sse` GET-only | `core/app.go:383-386`, `core/hydro.go:453-456`, `core/sse.go:205-208` | `TestNonGETMethodsAreRejected` `core/app_test.go:128` — endpoint method gates unpinned, → [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |

### Residual risk

- **The patch swap is an `innerHTML` sink by design** (`core/runtime.js:28`, D14). The
  patch is trusted server output; the entire defense is server-side contextual escaping
  (boundary 2). Accepted risk AR-1 below.
...
| Prototype validation | Registration rejects prototypes with non-nil reference-typed fields (they would alias across the per-request copies); `pathParam` tags must be exported string fields | `core/app.go:328-346`, `:289-303` | `TestRouteRejectsPrototypesWithSharedReferenceFields` `core/app_test.go:117`; `TestRouteRejectsPathParamTagOnNonStringField` `:474` — unexported-field branch unpinned, → [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |
...
- Escaping protects HTML contexts. It does not sanitize URLs used in `Redirect(...)`
  (guard or event) — an author redirecting to attacker-influenced input is an open
  redirect; v0.1 has no allowlist for redirect targets. Author responsibility, called out
  here so #30 probes it.
...
| Overlay renders diagnostics inert | The dev overlay sets diagnostic text via `textContent` — never `innerHTML` — precisely because diagnostics quote untrusted source | `core/dev.js:33` (design comment `:15-16`) | **None** → [#34](https://github.com/rmoralesthompson/Liquid/issues/34) |
...
| Control stream auth | CLI listens on `127.0.0.1` only; the URL path is a 16-byte `crypto/rand` hex token passed via `LIQUID_DEV_CONTROL` (`core/dev_on.go:33,59-65`); only that path is routed | `cmd/liquid/dev.go:375-391` | **None** (exercised e2e by `TestDevLoopEndToEnd` `cmd/liquid/dev_integration_test.go:22`, but bind/token unforgeability unasserted) → [#34](https://github.com/rmoralesthompson/Liquid/issues/34) |
| Dev streams bounded | Sessionless `?dev=1` SSE streams capped at 128, oldest evicted | `core/dev_on.go:48,69-77`; gate `core/sse.go:217-221` | **None** for the cap → [#34](https://github.com/rmoralesthompson/Liquid/issues/34); single-stream behavior `core/dev_wb_test.go:93` |
...
- **All framework logging goes through a pluggable `slog` handler** (`WithLogger`,
  `core/app.go:149-153`; default `slog.Default()` at `:171`). Every call site is
  `.Error(...)` — 16 sites total across `core/` and `cmd/liquid/`, each audited for #29.
  No `fmt.Println` exists in `core/` (currently unenforced by lint →
  [#35](https://github.com/rmoralesthompson/Liquid/issues/35)).
  Pinned: `TestRenderFailureIs500AndLoggedToInjectedLogger` `core/app_test.go:148`.
- **What reaches logs:** error text, request paths, panic values, action names. Verified
  **never** logged: session cookie values, CSRF tokens, the CSRF secret, request bodies.
  Client-facing `http.Error` strings are generic constants that never echo request data
  (`core/hydro.go:454-484`, `core/sse.go:206-212`, `core/app.go:384,490`).
- **Exception (open finding):** the per-instance hydro token is logged verbatim on two
  SSE error paths (`core/sse.go:104,109`) — the only credential that can reach the log
  sink — and no test pins token *absence* from logs anywhere →
  [#32](https://github.com/rmoralesthompson/Liquid/issues/32).
...
| Transforms on a parsed HTML node tree, not line/regex | **Architectural + indirect** | `x/net/html` at `compiler.go:177`; `compiler_test.go:288,181` — accepted risk AR-4 |
| Same-session events serialized, never concurrent on one instance | **Pinned** | `core/hydro_test.go:553,589` under `-race` |
| Session registry bounded (both caps, eviction) | **Pinned** (most thoroughly covered invariant) | `core/hydro_test.go:286,322,340,384,405,432`; `core/hardening_test.go:117` |
...
The Go-standards rule "no `fmt.Println` in core" (D24) has no lint enforcement →
[#35](https://github.com/rmoralesthompson/Liquid/issues/35).
...
| 4.3 | Regex line-based template parser (incl. infinite loop) | `x/net/html` node tree `compiler.go:177`; `compiler_test.go:288,181` |
...
- **AR-1 — `innerHTML` patch sink (D14).** The runtime swaps server-rendered HTML via
  `innerHTML` (`core/runtime.js:28`). Defense is entirely server-side contextual
  escaping (boundary 2, four pinning tests). Revisit when/if a morphdom-style merge
  lands (v0.2+ per D21).
...
| Gap | Tracked |
| --- | --- |
| Hydro token logged on SSE error paths; no log-hygiene (token-absence) pin | [#32](https://github.com/rmoralesthompson/Liquid/issues/32) |
| Production-boundary test gaps: endpoint method gates, refusal-order sequence, mid-body 413, guard-before-instantiation, nesting depth cap, `pathParam` unexported branch, default-limit literals | [#33](https://github.com/rmoralesthompson/Liquid/issues/33) |
| Dev-loop test gaps: overlay `textContent` pin, control-stream bind/token, 128-stream cap | [#34](https://github.com/rmoralesthompson/Liquid/issues/34) |
| `forbidigo` lint rule for D24's no-`fmt.Println`-in-core | [#35](https://github.com/rmoralesthompson/Liquid/issues/35) |
| Adversarial review of everything above | [#30](https://github.com/rmoralesthompson/Liquid/issues/30) (runs against this document) |
