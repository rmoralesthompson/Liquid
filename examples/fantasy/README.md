...
| Weekly matchup | `ui/matchup.{go,lsx}` | your head-to-head: live scores, projections, and an SVG win-probability bar pushed over SSE |
| League standings | `ui/standings.{go,lsx}` | `*goFor` ladder that re-sorts server-side as points-for tick up; your team and opponent highlighted |
| Around the league | `ui/around.{go,lsx}` | the week's other matchups, live scores, leader highlighted |
| Gameday ticker | `ui/ticker.{go,lsx}` | a rolling window of fictional news/stat items, pinned to the bottom, pushed as they "happen" |
